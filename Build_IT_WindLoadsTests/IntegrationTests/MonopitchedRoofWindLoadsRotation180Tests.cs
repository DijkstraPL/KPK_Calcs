...
{
    [TestFixture]
    public class MonopitchedRoofWindLoadsRotation180Tests
    {
        [Test]
        public void ExternalWindPressureForceCalculationsTest_Success()
        {
            ...
            var building = new MonopitchRoof(
                length, width, maxHeight, minHeight, MonopitchRoof.Rotation.Degrees_180);
            ...
            var flatRoofWindLoads = new MonopitchedRoofWindLoadsRotation180(
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(resultMax[Field.F], Is.EqualTo(-1.056).Within(0.001));
                Assert.That(resultMax[Field.G], Is.EqualTo(-0.572).Within(0.001));
                Assert.That(resultMax[Field.H], Is.EqualTo(-0.374).Within(0.001));

                Assert.That(resultMin[Field.F], Is.EqualTo(-1.056).Within(0.001));
                Assert.That(resultMin[Field.G], Is.EqualTo(-0.572).Within(0.001));
                Assert.That(resultMin[Field.H], Is.EqualTo(-0.374).Within(0.001));
            });
        }
    }
}
