...
namespace Build_IT_Application.ScriptInterpreter.Calculations.Queries.Test
{
        ...
        public class Handler : IRequestHandler<TestQuery, bool>
        {
                ...
                TestQuery request, CancellationToken cancellationToken)
            {
                ...
                var parameters = await _parameterRepository.GetAllParametersForScriptAsync(testData.ScriptId)
                    .ConfigureAwait(false);
                var parametersResource = _mapper.Map<IList<Parameter>, IList<CalculateParameterResource>>(parameters.ToList());
                var parametersResources = _mapper.Map<List<Parameter>, List<ParameterResource>>(parameters.ToList());

                var editableParameters = parametersResource.Where(p => (p.Context & SIP.ParameterOptions.Editable) != 0);

                var calculationParameters = SetValues(testData, editableParameters);

                try
                {
                    var scriptCalculator = new ScriptCalculator(parametersResources);
            ...
            private IEnumerable<CalculateParameterResource> SetValues(TestData testData, IEnumerable<CalculateParameterResource> editableParameters)
            {
                foreach (var editableParameter in editableParameters)
                {
                    if ((editableParameter.Context & SIP.ParameterOptions.Optional) == 0 ||
